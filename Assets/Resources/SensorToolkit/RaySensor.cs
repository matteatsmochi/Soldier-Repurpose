...
namespace SensorToolkit
{
	...
	public class RaySensor : Sensor
    {
        ...
        // The radius of the ray, set to zero to test with Physics.Raycast, values above zero will test with Physics.SphereCast
		...
		void testRayMulti()
		{
            ...
            if (Radius > 0f) 
            {
                hits = Physics.SphereCastAll(ray, Radius, Length, combinedLayers);
            } 
            else 
            {
                hits = Physics.RaycastAll(ray, Length, combinedLayers);
            }
